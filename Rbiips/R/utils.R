...
to_biips_vname <- function(var) {
  ### FIXME transform variable name. eg: x[1, ] => x[1,1:100]
    stopifnot(is.character(var), length(var) == 1, nchar(var) >
        0)
    ## remove spaces
    v <- gsub(" ", "", var)
    return(v)
...
parse_varname <- function(varname) {
    ...
    if (is.name(v)) {
        ...
        if (any(nchar(sapply(v, deparse)) == 0)) {
            ## We have to catch empty indices here or they will cause
            ## trouble below
            return()
        }
        for (i in 1:ndim) {
...
deparse_varname <- function(name, lower = NULL, upper = lower) {
    ...
    stopifnot(is.numeric(lower), is.numeric(lower), length(upper) ==
        length(lower), upper >= lower)
...
data_preprocess <- function(data) {
    ...
    if (length(ignored) > 0)
        warning("Ignored non numeric variables in data: ", paste(ignored,
            collapse = " "))
...
mklist <- function(names, env = parent.frame()) {
    ...
    n <- which(is.na(d))
    if (length(n) > 0) {
        stop("objects ", paste("'", names[n], "'", collapse = ", ",
            sep = ""), " not found")
    }
    return(d)
}
