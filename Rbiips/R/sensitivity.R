...
#' @param param_values  named list of numeric values. Each member is named after a parameter for which we
#'                      want to study sensitivity. Each member value is a vector or array whose last
...
biips_smc_sensitivity <- function(object, param_values, n_part,
    ...
    ## Get the number of parameter values to evaluate (the last
    ## dimension)
    ...
    Rbiips("message", paste("Analyzing sensitivity with", n_part,
        "particles"))
    ## progress bar
    ...
    for (k in 1:n_values) {
        ...
        for (i in 1:n_param) {
            var <- varnames[[i]]
            ind <- seq(from = (k - 1) * len[[i]] + 1, to = k *
                len[[i]])
            value[[var]] <- param_values[[i]][ind]
            dim(value[[var]]) <- dimen[[i]]

            ## change param value
            if (!.Call("change_data", model$ptr(), pn$names[[i]],
                pn$lower[[i]], pn$upper[[i]], value[[i]], FALSE,
                PACKAGE = "Rbiips"))
                stop("data change failed: invalid parameter. ",
                  var, " = ", paste0(value[i], collapse = " "))
        }

        ## get log prior
        log_prior <- 0
        for (i in 1:n_param) {
            log_p <- .Call("get_log_prior_density", model$ptr(),
                pn$names[[i]], pn$lower[[i]], pn$upper[[i]],
                PACKAGE = "Rbiips")
        ...
        if (!ok)
            stop("Failure running SMC forward sampler. parameter: ",
                paste(names(value), "=", value, sep = "", collapse = ";"))
    ...
    ## output --------
    out <- list(log_marg_like = log_marg_like, max_param = max_param,
        max_log_marg_like = max_log_marg_like, log_marg_like_pen = log_marg_like_pen,
        max_param_pen = max_param_pen, max_log_marg_like_pen = max_log_marg_like_pen)
