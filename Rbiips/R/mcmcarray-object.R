...
#'   See \code{\link[stats]{density}}, \code{\link[stats]{hist}}, \code{\link[stats]{table}}
#'
...
#' legend("topright", leg = c("true", "PMMH estimate"), lty = c(2, 1), col = c("green", "blue"), bty = "n")
#'
...
mcmcarray <- function(data = NA, dim = length(data), dimnames = NULL,
                      iteration = length(dim), chain = NA, name = "mcmcarray",
...
biips_density.mcmcarray <- function(x, bw = "nrd0", ...) {
  ...
  for (d in 1:len) {
    ...
    ind <- attr(x, "lower") + get_index(d, rep(0, length(attr(x, "lower"))), attr(x, "upper")-attr(x, "lower"))
...
biips_hist.mcmcarray <- function(x, main = NULL, xlab = NULL,
  ...
  for (d in 1:len) {
    ...
    out[[d]] <- hist(values, main = main_, xlab = rec(xlab,
                                                      d), ...)  # recycle arguments
    out[[d]]$xname <- xname
  }
...
plot.table.mcmcarray <- function(x, main = NULL, xlab = NULL,
  ...
  for (d in 1:length(x)) {
    plot(x[[d]], main = rec(main, d), xlab = rec(xlab, d),
         ylab = rec(ylab, d), ...)  # recycle arguments
  }
  invisible(NULL)
}
