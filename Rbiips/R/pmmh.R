...
pmmh_set_param.biips <- function(object, param_names, pn_param, 
    ...
    if (length(values) > 0) {
        ...
        for (i in 1:n_param) {
            ...
            ok <- Rbiips("change_data", console, pn_param$names[[i]], 
                pn_param$lower[[i]], pn_param$upper[[i]], values[[var]], 
                TRUE)
            if (!ok) 
                stop("Data change failed: invalid value for variable ", 
                  var)
        ...
        for (i in 1:n_param) {
            tryCatch({
                var <- param_names[[i]]
                sample_param[[var]] <- Rbiips("sample_data", 
                  console, pn_param$names[[i]], pn_param$lower[[i]], 
                  pn_param$upper[[i]], get_seed())
            }, error = function(e) {
                print(e)
...
biips_pmmh_init <- function(object, param_names, latent_names = c(), 
    ...
    stopifnot(is.numeric(alpha), length(alpha) == 1, alpha >= 
        0, alpha <= 1)
    ...
    ## check param_names
    stopifnot(is.character(param_names), length(param_names) > 
        0)
    ...
    if (length(rw_step) > 0) {
        stopifnot(length(rw_step) == n_param, sapply(rw_step, 
            is.numeric))
        stopifnot(sapply(rw_step, function(x) all(is.finite(x) && 
            x > 0)))
    ...
    state <- list(sample_param = sample_param, sample_latent = list(), 
        log_marg_like = -Inf, log_prior = -Inf)
    ...
    # target_prob = target acceptance probability. The default
    # seems to be a fairly robust optimal value. Comes from
    # Rosenthal 2009, Examples of Adaptives MCMC, p 16. In
    # one-dimensional case, we take 0.44 which is optimal in
    # certain settings.
    target_prob <- if (sample_len == 1) 
        0.44 else 0.234
    
    ## Init random walk stepsize for the part with diagonal
    ## covariance matrix
    if (length(rw_step) == 0) {
        ## default values
        rw_step <- lapply(sample_dim, function(x) 0.1/sqrt(sample_len) * 
            array(1, dim = x))
    } else {
        ## check dimensions
    ...
    # local initial parameters of the algorithm of adaptation of
    # the step of random walk in the PMMH algorithm
    rw <- list(n_rescale = n_rescale, n_cov = n_rescale/2, n_iter = 0, 
        ar_mean = 0, log_step = log_step, beta = beta, alpha = alpha, 
        target_prob = target_prob, dim = sample_dim, len = sample_len, 
        mean = c(), cov = c(), transform = list(), transform_inv = list(), 
        transform_lderiv = list())
    # we start learning the covariance matrix after n_cov
    # iterations
    ...
    for (i in 1:n_param) {
        ...
        if (transform) {
            tryCatch({
                support <- Rbiips("get_fixed_support", model$ptr(), 
                  pn_param$names[[i]], pn_param$lower[[i]], pn_param$upper[[i]])
            }, error = function(e) {
                print(e)
                stop("CANNOT GET FIXED SUPPORT OF VARIABLE ", 
                  var, ": BUG TO BE FIXED")
            })
            if (nrow(support) != len) 
                stop("support size does not match")
        } else {
            support <- matrix(c(rep(-Inf, len), rep(+Inf, len)), 
                nrow = len)
        }
        for (j in 1:len) {
            k <- k + 1
            ...
            if (is.finite(L)) {
                if (is.finite(U)) {
                  ...
                  rw$transform[[k]] <- function(x) log((x - L)/(U - 
                    x))  # direct transformation
                  rw$transform_inv[[k]] <- function(y) L + (U - 
                    L)/(1 + exp(-y))  # inverse transformation
                  rw$transform_lderiv[[k]] <- function(x) log(U - 
                    L) - log(x - L) - log(U - x)  # log derivative
                } else {
                  if (U < 0) 
                  ...
                  rw$transform_lderiv[[k]] <- function(x) -log(x - 
                    L)
                }
            } else {
                if (L > 0) 
    ...
    rw_transform <- function(sample, funtype = "transform") {
        ...
        funlist <- switch(funtype, transform = rw$transform, 
            inverse = rw$transform_inv, lderiv = rw$transform_lderiv)
        
        # concatenate all variables in a vector always in the order
        # of rw$dim
        sample_vec <- c(unlist(sample))
        
        out_vec <- mapply(FUN = function(x, fun) fun(x), sample_vec, 
            funlist)
    ...
    obj_pmmh <- list(model = function() {
        ...
        rw$log_step <<- rw$log_step + rw$alpha^(rw$n_iter) * 
            (rw$ar_mean - rw$target_prob)
        ...
        if (n_iter == n_cov + 1) {
            ...
            ## Recursive update of the empirical mean and covariance
            ## matrix
...
pmmh_rw_proposal <- function(object) {
    ...
    if (length(cov) == 0 || (n_iter < n_rescale) || (runif(1) < 
        beta)) {
        ...
        prop_vec_tr <- sample_vec_tr + 2.38/sqrt(rw_len) * cov_chol %*% 
            rnorm(rw_len)
    }
...
pmmh_one_update <- function(object, pn_param, n_part, rw_rescale, 
    ...
    # for (var in names(prop)) { if (any(is.na(prop[[var]]))) {
    ...
    for (i in 1:n_param) {
        ...
        ok <- Rbiips("change_data", console, pn_param$names[[i]], 
            pn_param$lower[[i]], pn_param$upper[[i]], prop[[var]], 
            TRUE)
        
        if (!ok) {
            ## DATA CHANGE FAILED: proposed parameter value might be out
            ## of bounds ?  TODO: double check this is the case and there
            ## is no other reason.
            log_prior_prop <- -Inf
        ...
        log_p <- Rbiips("get_log_prior_density", console, pn_param$names[[i]], 
            pn_param$lower[[i]], pn_param$upper[[i]])
        
        log_p <- log_p - sum(abs(lderiv[[var]]))
        
        if (is.na(log_p)) 
            stop("Failed to get log prior density: node ", var, 
                " is not stochastic.")
    ...
    if (log_prior_prop == -Inf) {
        ...
        if (!ok) {
            ...
            log_marg_like_prop <- Rbiips("get_log_norm_const", 
                console)
            if (is.nan(log_marg_like_prop) || log_marg_like_prop == 
                Inf) 
                ### TODO error or n_fail increment ?
            stop("Failed to compute log marginal likelihood: ", 
                log_marg_like_prop)
        }
...
pmmh_algo.pmmh <- function(object, n_iter, n_part, return_samples, 
    ...
    stopifnot(is.numeric(max_fail), length(max_fail) == 1, max_fail >= 
        0)
    ...
    ## FIXME if (!rw.adapt) { ## check adaptation if
    ## (object$.rw_adapt() && !object$.rw_check_adapt())
    ## message('NOTE: Stopping adaptation of the PMMH random
    ...
    if ("a" %in% output) 
        out$info$accept_rate <- mcmcarray(dim = c(1, n_samples))
    if ("s" %in% output) {
        len <- length(object$rw_step())
        out$info$rw_step <- mcmcarray(dim = c(len, n_samples))
    }
    ...
    for (i in 1:n_iter) {
        out_step <- pmmh_one_update(object, pn_param = pn_param, 
            n_part = n_part, rw_rescale = rw_rescale, rw_learn = rw_adapt, 
            ...)
        ...
        if (n_fail > max_fail) 
            stop("Number of failures exceeds max_fail: ", n_fail, 
                " failures.")
        ...
        if ((i%%thin) == 0) {
            ...
            if ("p" %in% output) 
                out$log_marg_like_pen[ind_sample] <- log_marg_like + 
                  log_prior
            if ("l" %in% output) 
