...
data <- list(tmax = tmax, p = p, logtau_true = logtau_true, logtau = logtau)
...
out_smc <- biips_smc_samples(model, c('x', 'c[2:10]'), n_part, type = 'fs', rs_thres = 0.5,
  rs_type = 'stratified')
...
is.smcarray(out_smc[['c[2:10]']]$f)
names(out_smc[['c[2:10]']]$f)
out_smc[['c[2:10]']]$f
out_smc[['c[2:10]']]$s
biips_diagnosis(out_smc[['c[2:10]']]$f)
biips_summary(out_smc[['c[2:10]']]$f)
par(mfrow = c(2, 2))
...
matlines(matrix(unlist(summ_smc_x$f$quant), tmax), lty = 2, col = 'blue')
matlines(matrix(unlist(summ_smc_x$s$quant), tmax), lty = 2, col = 'red')
legend('topright', leg = c('true', 'SMC filtering estimate', 'SMC smoothing estimate'),
  lty = 1, col = c('green', 'blue', 'red'), bty = 'n')

barplot(2.5*(model$data()$c_true==1), col = 'green', border = NA, space = 0, ylim=c(0,3),
        xlab='t', ylab='c[t]==1', axes = FALSE)
axis(1, at=1:tmax-.5, labels=1:tmax)
axis(2, line = 1, at=c(0,3), labels=NA)
barplot(2*(model$data()$c_true==1), col = 'white', border = NA, space = 0, axes = FALSE, add = TRUE)
text(tmax/2, 2.75, 'true')
barplot(1.5*(summ_smc_c$f$mode==1), col = 'blue', border = NA, space = 0, axes = FALSE, add = TRUE)
barplot(1*(summ_smc_c$f$mode==1), col = 'white', border = NA, space = 0, axes = FALSE, add = TRUE)
text(tmax/2, 1.75, 'SMC filtering mode')
barplot(.5*(summ_smc_c$s$mode==1), col = 'red', border = NA, space = 0, axes = FALSE, add = TRUE)
text(tmax/2, .75, 'SMC smoothing mode')
...
model <- biips_model(modelfile, c('tmax', 'p', 'logtau_true', 'logtau'))
...
t <- 5
plot(out_pimh$x[t, ], type = 'l', col = 'blue', xlab = 'PIMH iteration', ylab = paste0('x[',t,']'))
points(0, model$data()$x_true[t], pch = 17, col = 'green')

par(mfrow = c(2, 2))
plot(model$data()$x_true, type = 'l', col = 'green', xlab = 't', ylab = 'x[t]')
lines(summ_pimh_x$mean, col = 'blue')
matlines(matrix(unlist(summ_pimh_x$quant), tmax), lty = 2, col = 'blue')
legend('topright', leg = c('true', 'PIMH estimate'), lty = c(2, 1), col = c('green',
  'blue'), bty = 'n')

barplot(1.5*(model$data()$c_true==1), col = 'green', border = NA, space = 0, ylim=c(0,2),
        xlab='t', ylab='c[t]==1', axes = FALSE)
axis(1, at=1:tmax-.5, labels=1:tmax)
axis(2, line = 1, at=c(0,2), labels=NA)
barplot(1*(model$data()$c_true==1), col = 'white', border = NA, space = 0, axes = FALSE, add = TRUE)
text(tmax/2, 1.75, 'true')
barplot(.5*(summ_pimh_c$mode==1), col = 'red', border = NA, space = 0, axes = FALSE, add = TRUE)
text(tmax/2, .75, 'PIMH mode')
...
model <- biips_model(modelfile, c('tmax', 'p', 'logtau_true'))

n_part <- 50
obj_pmmh <- biips_pmmh_init(model, 'logtau', latent_names = c('x', 'c[2:10]'), inits = list(logtau = -2))  # Initialize
is.pmmh(obj_pmmh)
...
matlines(matrix(unlist(summ_pmmh_x$quant), tmax), lty = 2, col = 'blue')
...
barplot(1*(model$data()$c_true==1), col = 'white', border = NA, space = 0, axes = FALSE, add = TRUE)
text(tmax/2, 1.75, 'true')
barplot(.5*(summ_pmmh_c$mode==1), col = 'blue', border = NA, space = 0, axes = FALSE, add = TRUE)
text(tmax/2, .75, 'PMMH mode')
