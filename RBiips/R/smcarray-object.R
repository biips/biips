...
plot.summary.smcarray <- function(x, type = "l", lty = 1:5, lwd = 2, col = 1:6, xlab = "offset", 
    ...
    stat.names <- names(x)[!(names(x) %in% c("2nd mt.", "Var.", "SD", "3rd mt.", 
        "Skew.", "4th mt.", "Kurt.", "drop.dims", "name", "type"))]
    ...
    if (missing(sub)) 
        sub <- paste("Marginalizing over:", paste(paste(names(x$drop.dims), "(", 
...
density.smcarray <- function(x, bw = "nrd0", adjust = 1, subset, ...) {
    ...
    for (d in 1:len) {
        ...
        ans[[varname]] <- list(density = dens, name = varname, type = x$type, n_part = n.part, 
