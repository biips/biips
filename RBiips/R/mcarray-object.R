mcarray <- function(data = NA, dim = length(data), dimnames = NULL, iteration = length(dim), 
  chain = NA) {
...
summary.mcarray <- function(object, FUN, ...) {
  ...
  ans <- list(stat = apply(object, which(!drop.dims), FUN, ...), drop.dims = dim(object)[drop.dims])
...
print.summary.mcarray <- function(x, ...) {
  ...
  if (length(x$drop.dims) > 0) {
    cat("\nMarginalizing over:", paste(paste(names(x$drop.dims), "(", x$drop.dims, 
      ")", sep = ""), collapse = ","), "\n")
  }
}
...
as.mcmc.list.mcarray <- function(x, ...) {
  ...
  if (length(which.chain) == 0) {
    ...
    x <- aperm(x, perm)
    for (i in 1:nchain) {
      ans[[i]] <- mcmc(matrix(x[1:len + (i - 1) * len], nrow = niter, byrow = TRUE))
    }
    ans <- mcmc.list(ans)
  }
  return(ans)
