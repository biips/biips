...
##' @export
pmmh.init <- function(object, ...) UseMethod("pmmh.init")
...
one.pmmh.update.biips <- function(object, param.names, latent.names = c(), pn.param, 
    sample, log.prior, log.marg.like, n_part, rw.rescale, rw.learn, rw.rescale.type, 
    ...) {
    n.fail <- 0
    ...
    ## Acceptance rate
    accept.rate <- min(1, exp(log.marg.like.prop - log.marg.like + log.prior.prop - 
        log.prior))
...
pmmh.update.biips <- function(object, param.names, n_iter, n_part, max_fail = 0, 
    ...
    ### Initialize -------------------------------
    out <- pmmh.init.biips(object, param.names = param.names, n_part = n.part, inits = inits, 
        ...)
    ...
    for (i in 1:n_iter) {
        ...
        out <- one.pmmh.update.biips(object, param.names = param.names, pn.param = pn.param, 
            ...
            n_part = n.part, rw.rescale = rw.rescale, rw.learn = rw.learn, rw.rescale.type = rw.rescale.type, 
            ...)
...
pmmh.samples <- function(object, param.names, latent.names = c(), n_iter, thin = 1, 
    ...
    for (i in 1:n_iter) {
        out <- one.pmmh.update.biips(object, param.names = param.names, latent.names = latent.names, 
            pn.param = pn.param, sample = sample, log.prior = log.prior, log.marg.like = log.marg.like, 
            n_part = n.part, rw.rescale = rw.rescale, rw.learn = rw.learn, rw.rescale.type = rw.rescale.type, 
            ...)
        sample <- out$sample
        log.prior <- out$log.prior
