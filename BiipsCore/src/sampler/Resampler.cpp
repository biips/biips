...
namespace Biips
{
  ...
  void Resampler::Resample(Types<Particle>::Array & particles, Scalar & sumOfWeights, Rng & rng)
  {
    ...
    for (Size i=0, j=0; i<nParticles_; ++i)
    {
      if (count_[i]>0)
      {
        ...
        while (count_[i]>1)
        {
          while (count_[j]>0) ++j; // find next free spot
  ...
  protected:
    MultinomialResampler() : BaseType("multinomial") {}

    virtual void resample(Scalar sumOfWeights, Rng & rng);

  public:
    static BaseType::Ptr Instance() { static BaseType::Ptr p_instance(new SelfType()); return p_instance; }
  };
  ...
  protected:
    ResidualResampler() : BaseType("residual") {}
  ...
  protected:
    StratifiedResampler() : BaseType("stratified") {}
  ...
  protected:
    SystematicResampler() : BaseType("systematic") {}
  ...
  void ResidualResampler::resample(Scalar sumOfWeights, Rng & rng)
  {
    ...
    Size n_multinomial = nParticles_ - std::accumulate(count_.begin(), count_.end(), 0);
