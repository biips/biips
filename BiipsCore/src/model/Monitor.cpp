...
namespace Biips
{
  ...
  void FilterMonitor::SetWeights(const Types<Particle>::Array & particles, Scalar ess, Scalar sumOfWeights, Scalar logNormConst)
  {
    checkSwapped();
  ...
  void FilterMonitor::SetNodeValues(NodeId nodeId, const Types<Particle>::Array & particles)
  {
    if (Contains(nodeId))
