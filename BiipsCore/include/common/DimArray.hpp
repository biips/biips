...
namespace Biips
{
  ...
  public:
     ...
     */
    explicit DimArray(size_type n) : BaseType(n) {}
    /*!
     * Creates a DimArray with n copies of s.
     * \param n number of dimensions
     * \param s size of the dimensions
     */
    DimArray (size_type n, value_type s) : BaseType(n, s) {}
    /*!
    ...
    DimArray (InputIterator first, InputIterator last) : BaseType(first, last) {}

    //! The assignment operator
    DimArray & operator= (const DimArray & from) { BaseType::operator=(from); return *this; };

    //! Number of dimensions accessor
    /*!
     * @return the size of the DimArray
     */
    size_type NDim() const { return size(); }
     ...
     */
    value_type Length() const { return empty() ? 0 : std::accumulate(begin(), end(), value_type(1), std::multiplies<value_type>()); }
    /*!
     * Indicates whether the DimArray corresponds to a scalar object,
     * i.e. containing only one element.
     */
    Bool IsScalar() const { return (Length() == 1); } // FIXME
    /*!
     ...
     */
    Bool IsMatrix() const { return (size() == 2); }
    /*!
