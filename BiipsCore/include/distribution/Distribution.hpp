...
namespace Biips
{
  ...
  protected:
    ...
    virtual Bool checkParamValues(const MultiArray::Array & paramDims) const = 0;
    virtual DimArray dim(const Types<DimArray::Ptr>::Array & paramDims) const = 0;
    virtual MultiArray sample(const MultiArray::Array & paramValues,
        const MultiArray::Pair & boundValues, Rng & rng) const = 0;
    virtual Scalar logDensity(const MultiArray & x, const MultiArray::Array & paramValues,
        const MultiArray::Pair & boundValues) const = 0;
    virtual MultiArray::Pair unboundedSupport(const MultiArray::Array & paramValues) const = 0;
  ...
  public:
    ...
    Scalar LogDensity(const MultiArray & x, const MultiArray::Array & paramValues,
    ...
    virtual Bool CanBound() const { return false; }
    virtual Bool IsDiscreteValued(const Flags & mask) const { return false; }
    virtual Bool CheckParamDiscrete(const Flags & mask) const { return true; }
    virtual Bool IsLocationParameter(Size i) const { return false; }
    virtual Bool IsScaleParameter(Size i) const { return false; }
    virtual Size DegreeOfFreedom(const Types<DimArray::Ptr>::Array & paramDims) const { return Dim(paramDims).Length(); }
