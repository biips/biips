...
#+ setup, include=FALSE
library(knitr)
opts_chunk$set(cache=TRUE, comment=NA, warning=FALSE, background='white', fig.show='hold', tidy=FALSE, pars=TRUE)
...
cat(readLines('hmm_1d_nonlin.bug'), sep = "\n")
...
#+ eval=FALSE
install.packages('path/to/R/biips_x.x.x.ext', repos=NULL)
...
for (i in 1:length(out_smc$x$f$conditionals)) {
  ...
  cat(paste(out_smc$x$f$conditionals[[i]], sep=', '), '\n');
}

#' while the smoothing distributions are $\pi(x_{t}|y_{1:t_{max}})$, for $t=1,\ldots,t_{max}$.
#+ eval=FALSE
cat('Smoothing distributions:\n')
for (i in 1:length(out_smc$x$s$conditionals)) {
  cat(out_smc$x$s$iterations[i], ': x[', i, '] | ', sep='');
  cat(paste(out_smc$x$s$conditionals[[i]], sep=', '), '\n');
}
...
palette(lightcols)
polygon(xx, yy, col='blue', border=NA)
palette('default')
...
legend('topright', leg=c('95 % credible interval', 'Filtering mean estimate', 'True value'),
       col=c('blue','green3','blue'), lwd=c(NA,3,2), pch=c(15,NA,NA), pt.cex=c(2,1,1), bty='n')
...
legend('topright', leg=c('95 % credible interval', 'Smoothing mean estimate', 'True value'),
       col=c('red','green3','red'), lwd=c(NA,3,2), pch=c(15,NA,NA), pt.cex=c(2,1,1), bty='n')
...
legend('topright', leg=c('95 % credible interval', 'PIMH mean estimate', 'True value'),
       col=c('blue','green3','blue'), lwd=c(0,3,2), pch=c(15,NA,NA), pt.cex=c(2,1,1),
       bty='n')
