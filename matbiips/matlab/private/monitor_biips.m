...
if (~isempty(indices{1})) % filtering
  matbiips('set_filter_monitors', p, names, lower, upper);
end 
if (~isempty(indices{2})) % smoothing
%     p, names, lower, upper
  matbiips('set_gen_tree_smooth_monitors', p, names, lower, upper);
end  
if (~isempty(indices{3})) %backward_smoothing
  matbiips('set_backward_smooth_monitors', p, names, lower, upper);
end 
