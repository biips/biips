...
\section{introduction}
  ...
  Here, we have enumerated all the "internal" functions that one could call using the \texttt{inter\_biips} function. These functions are not intended
...
ans =
 ...
 \texttt{GET\_GEN\_TREE\_SMOOTH\_MONITORS} dumps the smooth monitors into the \texttt{smooth\_monitors} double nested struct: for each monitored variable , we have
  ...
  \begin{itemize}
   ...
   \item \texttt{weigths}: vector of weights (same dimensions as values)
   \item \texttt{ess}: vector of effective  sample size ( size: $N_{\mbox{space}}$)
   \item \texttt{discrete}: vectors of booleans if the variable if discrete (vectors of zeros otherwise)
   \item \texttt{name}: name of the variable
   \item \texttt{lower}: vector of lower bounds
   \item \texttt{upper}: vector of upper bounds
...
\section{change\_data}
 ...
 \begin{lstlisting}
 change_ok = inter_biips('change_data', id, var, lower, upper, values, mcmc)
 \end{lstlisting}
...
\section{is\_gen\_tree\_smooth\_monitored}
 ...
 \begin{lstlisting}
  ok = inter_biips('is_gen_tree_smooth_monitored', id, names, lower, upper, released)
 \end{lstlisting}
...
\section{set\_sampled\_gen\_tree\_smooth\_particle}
 ...
 \begin{lstlisting}
  sampled_value = inter_biips('set_sampled_gen_tree_smooth_particle', id, values)
 \end{lstlisting}
...
\section{add\_function}
 ...
 \begin{lstlisting}
  inter_biips('add_function', func, nb_args, fun_dim, fun_eval, fun_check, fun_discrete)
 \end{lstlisting}
