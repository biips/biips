...
namespace Biips
{
  ...
  const String ConjugateNormalVarLinear::NAME_ =
      "Conjugate Normal (with known variance and linear mean function)";
  ...
  void ConjugateNormalVarLinear::sample(const StochasticNode & node)
  {
    ...
    nodeValuesMap()[nodeId_]
        = DNormVar::Instance()->Sample(post_param_values,
                                       NULL_MULTIARRAYPAIR,
                                       *pRng_).ValuesPtr();
    ...
    logIncrementalWeight_
        = DNormVar::Instance()->LogDensity(MultiArray(prior_mean),
