...
//  void ModelTest::getValues(std::istream & is, DataType::Array & dataArray, const DimArray::Ptr dimPtr, Size dim, char separator) const
//  {
//    using namespace std;
//    String value_str;
//    Scalar value;
//    Bool allocate = dataArray.empty();
//    Size t = 0;
//    while (is.good())
//    {
//      getline(is, value_str, separator);
//      istringstream(value_str) >> value;
//      if (allocate)
//        dataArray.push_back(DataType(dimPtr));
//      dataArray[t][dim] = value;
//      ++t;
//    }
//  }
//
//  void ModelTest::getLine(std::istream & is, DataType::Array & dataArray, const DimArray::Ptr dimPtr, Size dim, char separator) const
//  {
//    using namespace std;
//    String value_str;
//    Scalar value;
...
//  void ModelTest::inputValues(DataType::Array & values, Size varLen, const DimArray::Ptr varDimPtr, std::istream & is) const
//  {
//    values.SetPtr(DataType::Array(varLen));
//    for (Size k=0; k<varLen; ++k)
//      values[k].SetPtr(varDimPtr).AllocValue();
//    for (Size dim=0; dim<varDimPtr->Length(); ++dim)
//      for (Size k=0; k<varLen; ++k)
//        is >> values[k].Value()[dim];
//  }
