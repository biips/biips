...
namespace Biips
{
  ...
  void Plot::AddCurve(const DataType::Array & x, const DataType::Array & y, String name, const QColor & color, uint width, Qt::PenStyle style, uint symbolSize, QwtSymbol::Style symbol)
  {
    QwtArray<Scalar> x_data(x.size());
    QwtArray<Scalar> y_data(y.size());
    Types<DataType>::ConstIterator it_x = x.begin();
    Types<DataType>::ConstIterator it_y = y.begin();
    Size i = 0;
    while( (it_x != x.end()) && (it_y != y.end()) )
    {
  ...
  void Plot::AddHistogram(const DataType::Array & x, const DataType::Array & y, String name, const QColor & color)
  {
    QwtArray<Scalar> x_data(x.size());
    QwtArray<Scalar> y_data(y.size());
    Types<DataType>::ConstIterator it_x = x.begin();
