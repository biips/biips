...
namespace Biips
{
  ...
  : BaseType(argc, argv, NAME_, verbose, showMode, os), precFlag_(precFlag)
  {
    ...
    dataTypeParamMap_["P.0"].SetPtr(dimArrayMap_["P.0"]).AllocValue(1.0);
    dataTypeParamMap_["F"].SetPtr(dimArrayMap_["F"]).AllocValue(1.0);
    dataTypeParamMap_["B"].SetPtr(dimArrayMap_["B"]).AllocValue(0.0);
    dataTypeParamMap_["Q"].SetPtr(dimArrayMap_["Q"]).AllocValue(1.0);
    dataTypeParamMap_["H"].SetPtr(dimArrayMap_["H"]).AllocValue(1.0);
    dataTypeParamMap_["R"].SetPtr(dimArrayMap_["R"]).AllocValue(0.5);
...
//  }
  ...
  void HmmMNormalLinear::BuildModelGraph()
  {
    ...
    NodeId mean_x0 = pModelGraph_->AddConstantNode(dataTypeParamMap_["mean.x.0"]);
  ...
  void HmmMNormalLinear::PlotResults(const String & plotFileName) const
  {
    ...
    if (dim_x == 1)
    {
      ...
      if (dim_y == 1)
        results_plot.AddCurve(time_y, y_obs, "observation", Qt::gray, 2, Qt::NoPen, 8, QwtSymbol::XCross);
