...
namespace smc {
  ...
  template <class Space> 
    class sampler
    {
    //private: /* Modified on 04/03/2010 by Adrien Todeschini */
    protected: /* Modified on 04/03/2010 by Adrien Todeschini : protected instead of private */
    ...
    public:
      ...
      virtual ~sampler(); /* Modified on 19/03/2010 by Adrien Todeschini : added virtual */
  ...
  sampler<Space>::sampler(long lSize, HistoryType htHM)
  {
    ...
    //htHistoryMode = SMC_HISTORY_RAM; /* Modified on 26/03/2010 by Adrien Todeschini */
    htHistoryMode = htHM; /* Modified on 26/03/2010 by Adrien Todeschini : arg htHM instead of default constant */
  ...
  sampler<Space>::~sampler()
  {
	 ...
	 delete [] uRSCount;
       if(uRSIndices)
	 delete [] uRSIndices;
       if(pParticles) /* Modified on 04/03/2010 by Adrien Todeschini */
         delete [] pParticles; /* Modified on 04/03/2010 by Adrien Todeschini : added pParticles deallocation */
  }
  ...
  void sampler<Space>::Resample(ResampleType lMode)
  {
    ...
    //for (unsigned int i=0, j=0; i<N; ++i) { /* Modified on 30/09/2010 by Adrien Todeschini */
    for (unsigned int i=0, j=0; int(i)< N; ++i) { /* Modified on 30/09/2010 by Adrien Todeschini */
      if (uRSCount[i]>0) { 
        uRSIndices[i] = i;
        while (uRSCount[i]>1) { 
    ...
    // for(int i = 0; i < N ; ++i) { /* Modified on 30/09/2010 by Adrien Todeschini */
