function [errors_filter_all, errors_smooth_all, log_norm_const_bench_all, models] = run_bench_all_compiler(model_ids, n_part, ess_thres, n_smc, results_file_names)
...
%     mutation_names = {'prior', 'optimal', 'bad'};
    ...
    monitored_var_names = {'x'};
    
    model_name = sprintf('HMM 1D linear gaussian, t = 0:%d', t_max);
    
    model = {model_const, model_dim, sample_param, sample_obs, bench, smc, mutation_names, model_name, const_names, var_names, model_file, monitored_var_names, dot_file};
    models = [models, {model} ];
end
...
%     fprintf(fid, 'period = %g\n', period);
    ...
    % print dot file
    fprintf(fid, '#dot-file = %s\n', dot_file);
    
    % print monitored variables
    fprintf(fid, 'monitor =');
    for i=1:length(monitored_var_names)
       fprintf(fid, ' %s', monitored_var_names{1});
    end
    fprintf(fid, '\n');
    ...
    % print sampled data
    if (size(x_gen, 1) == 1)
        fprintf(fid, '%s =', [var_names{1} '.true']);
        ...
        for i=1:size(x_gen,1)
            fprintf(fid, '%s.true.%d =', var_names{1}, i);
            for j=1:size(x_gen,2)
